...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { ProcessPromptAction, LoadLocalFileAction, PlayAudioAction };
}

/**
 * PlayAudioAction - Play audio from blackboard media
 * Returns RUNNING until playback ends, then SUCCESS or FAILURE
 */
class PlayAudioAction extends b3.Action {
    tick(blackboard) {
        const nodeId = this.id || this.title;
        const executionKey = `_exec_${nodeId}`;
        const isAlreadyExecuting = blackboard.get(executionKey);
        ...
        if (isAlreadyExecuting) {
            ...
            const result = blackboard.get(resultKey);
            if (result !== undefined) {
                blackboard.set(resultKey, undefined);
                blackboard.set(executionKey, false);
                return result;
            }
            return b3.Status.RUNNING;
        ...
        if (!inputKey) {
            console.error('PlayAudioAction: No inputKey specified');
            return b3.Status.FAILURE;
        }

        const slot = blackboard.get(inputKey);
        const mediaArr = slot ? slot.media : null;
        if (!mediaArr || !Array.isArray(mediaArr) || mediaArr.length === 0) {
            console.error(`PlayAudioAction: No audio data in blackboard key "${inputKey}"`);
            return b3.Status.FAILURE;
        }
        ...
        if (!b64) {
            console.error(`PlayAudioAction: Audio content is empty for key "${inputKey}"`);
            return b3.Status.FAILURE;
        }
        ...
        if (window.app) {
            window.app.outputMessage(`🔊 Playing audio from blackboard key "${inputKey}"`);
        }
        ...
        audio.play().catch(() => {
            if (window.app) {
                window.app.outputMessage(`⚠️ Autoplay blocked for "${inputKey}", click play on the player`);
            }
        });
