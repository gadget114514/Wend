...
class OpenCodeProvider {
    ...
    async call(req) {
        ...
        const raw = await httpRequest(
            ...
            body);
        if (!raw) throw new Error(`OpenCode API Error\nProvider: opencode\nModel: ${req.model}\nURL: ${url}\nError: Empty response received\nPossible causes: Invalid API key, network connectivity issue, or API endpoint unavailable`);
        const j = JSON.parse(raw);
        if (j.error) throw new Error(`OpenCode API Error\nProvider: opencode\nModel: ${req.model}\nURL: ${url}\nError: ${j.error.message}\nError type: ${j.error.type || 'unknown'}\nError code: ${j.error.code || 'unknown'}`);
        return { content: j.choices?.[0]?.message?.content ?? '[OpenCode: no content]', model: req.model, requestUrl: url, requestBody: body };
    }
